...
export function ModalVerifyCode({
  ...
  const handleRefreshToken = async () => {
    try {
      ...
      if (result) {
        ...
        setSubmitError(t("verifyRefreshFailed"));
      }
    } catch (error) {
      console.error("Error refreshing token:", error);
      setSubmitError(t("verifyRefreshError"));
    } finally {
  ...
  const onSubmit = async () => {
    ...
    if (token.length !== 6) {
      setSubmitError(t("verifyPinRequired"));
      return;
    }
    if (timeLeft <= 0) {
      setSubmitError(t("verifyCodeExpired"));
      return;
    }
    try {
      setIsSubmitting(true);
      setSubmitError("");
      const result = await verifyCode({ username, code: token, mode });
      if (result) {
        if (handleVerify) handleVerify();
        handleClose();
      } else setSubmitError(t("verifyInvalidCode"));
    } catch (error) {
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-sm font-mono font-bold ${timeLeft <= 60 ? "text-red-500" : timeLeft <= 300 ? "text-orange-500" : "text-green-500"}`}
            >
              {formatTime(timeLeft)}
            </span>
          </div>
        ...
        <div className="text-xs text-gray-500 text-center">
          {timeLeft > 0 ? (
            ...
            <span className="text-red-500">{t("verifyCodeExpired")}</span>
          )}
        </div>
      </div>
