...
export function ModalForgetPassword({ isOpen, onClose }: TProps) {
  ...
  const schema = yup.object().shape({
    password: yup
      .string()
      .min(6, t("resetPasswordMinLength"))
  ...
  const {
    ...
    reset: resetForm,
    watch
  } = useForm<TResetPassword>({ resolver: yupResolver(schema) });

  const passwordValues = watch();

  useEffect(() => {
    if (email.trim() && email !== debouncedEmail) setStatus("loading");
  ...
  const onSubmit = async (data: TResetPassword) => {
    ...
    if (token.length !== 6) {
      setSubmitError(t("resetPasswordPinRequired"));
      return;
    }
    try {
      ...
      else setSubmitError(t("resetPasswordInvalidCode"));
    } catch (error) {
      console.error("Error resetting password:", error);
      setSubmitError(t("resetPasswordError"));
    } finally {
  ...
  const ContentModal = (
    <div className="space-y-4">
      ...
      {status === "error" && (
        <p className="text-sm text-red-500">
          ⚠️ {t("resetPasswordCheckEmailError")}
        </p>
      )}
      ...
      {status === "valid" && (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <p className="text-sm text-green-500">
            ✅ {t("resetPasswordEmailValid")}
          </p>

          <div className="space-y-3">
            <label className="block text-sm font-medium text-black">
              {t("resetPasswordVerificationCode")}
